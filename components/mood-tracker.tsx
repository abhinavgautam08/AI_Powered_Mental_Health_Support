...
export function MoodTracker({ emotions }: MoodTrackerProps) {
  ...
  // Filter out duplicate emotions that occur within 5 seconds of each other
  const filteredEmotions = emotions.reduce((acc: typeof emotions, current, index) => {
    // Always include the first emotion
    if (index === 0) {
      return [current]
    }
    ...
    // Check if this emotion is different from the previous one or occurred more than 5 seconds later
    if (
      current.emotion !== prev.emotion ||
      differenceInSeconds(new Date(current.timestamp), new Date(prev.timestamp)) > 5
    ) {
      acc.push(current)
    }
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <div>
        <h3 className="font-medium text-sm sm:text-base">{t.currentMood}</h3>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                {emotions.length > 0
                  ? `${t.lastUpdated} ${format(new Date(emotions[emotions.length - 1].timestamp), "h:mm a")}`
                  : t.noDataAvailable}
              </p>
            </div>
      ...
      <div>
        <h3 className="font-medium text-sm sm:text-base">{t.moodHistory}</h3>
      ...
      {/* Recent Mood Changes - Now using filtered emotions */}
      {filteredEmotions.length > 1 && (
        <div>
          <h3 className="font-medium text-sm sm:text-base">{t.recentMoodChanges}</h3>
